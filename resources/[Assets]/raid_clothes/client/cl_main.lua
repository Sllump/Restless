...
RegisterNUICallback('escape', function(data, cb)
    ...
    if shouldSave and currentPrice > 0 then
        local purchaseSuccess = RPC.execute("clothing:purchase", currentPrice, paymentType)
    ...
    if data.paymentType == "dev" then
        SaveDev(shouldSave, true, newFadeStyle)
    else
...
function SaveDev(save, close, newFadeStyle)
    local data = GetCurrentPed()
    local meta = { data = json.encode(data), _hideKeys = { "data" } }
    TriggerEvent("player:receiveItem", "customclothing", 1, false, meta, meta)
...
local function listenForKeypress(zoneName, zoneData, isFree)
    listening = true
    Citizen.CreateThread(function()
        local priceWithTax = RPC.execute("PriceWithTaxStrings", zoneData.basePrice, "Services")
...
AddEventHandler('raid_clothes:openClothing', function(pDontShowBarber, pShouldCost)
    if pShouldCost ~= nil and pShouldCost then
        local priceWithTax = RPC.execute("PriceWithTaxStrings", MenuData.clothing_shop.basePrice, "Services")
...
RegisterNetEvent('sk1c2:clothing:admin', function()
    OpenMenu("clothing_shop")
end)
