...
local function AddMethod(player)
    ...
    function player.networkVar(self, var, data)
        ...
        TriggerClientEvent("arp-base:networkVar", GetUser(self):getVar("source"), var, data)
    end
    ...
    function player.addMoney(self, amt)
        ...
        Void.DB:UpdateCharacterMoney(GetUser(self), characterId, cash, function(updatedMoney, err) 
            if updatedMoney then
                ...
                -- exports["arp-base"]:AddLog("Cash Added", GetUser(self), "Money added to user, amount: " .. tostring(amt))
            end
        end)
    end
    ...
    function player.removeMoney(self, amt)
        ...
        Void.DB:UpdateCharacterMoney(GetUser(self), characterId, cash, function(updatedMoney, err) 
            if updatedMoney then
                ...
                TriggerClientEvent("banking:updateCash", GetUser(self).source, GetUser(self):getCash(), amt)
                -- exports["arp-base"]:AddLog("Cash Removed", GetUser(self), "Money removed from user, amount: " .. tostring(amt))
            end
        end)
    end
    ...
    function player.removeBank(self, amt)
        ...
        Void.DB:UpdateCharacterBank(GetUser(self), characterId, bank, function(updatedMoney, err) 
            if updatedMoney then
                ...
                -- exports["arp-base"]:AddLog("Bank Removed", GetUser(self), "Bank removed from user, amount: " .. tostring(amt))
            end
        end)
    end
    ...
    function player.addBank(self, amt)
        ...
        Void.DB:UpdateCharacterBank(GetUser(self), characterId, bank, function(updatedMoney, err) 
            if updatedMoney then
                ...
                TriggerClientEvent("banking:updateBalance", GetUser(self).source, GetUser(self):getBalance(), amt)
                -- exports["arp-base"]:AddLog("Bank Added", GetUser(self), "Bank added to user, amount: " .. tostring(amt))
            end
        end)
    end
...
AddEventHandler("playerDropped", function(reason)
    ...
    local pUser = exports["arp-base"]:getModule("Player"):GetUser(src)
    ...
    if userjob == "police"or userjob == "sheriff" or userjob == "state" then
        exports['arp-emergencyjobs']:DisconnectJobUpdater(userjob)
        TriggerEvent("arp-emergencyjobs:log:update", src, "clockoff")
    elseif userjob == "ems" then
        exports['arp-emergencyjobs']:DisconnectJobUpdater(userjob)
    end

    Void.Users[src] = nil
    TriggerEvent('arp-base:playerDropped', src, user)
    
    exports['arp-admin']:addPlayerLog('Player Left', GetPlayerIdentifier(src, 0), pSteamName..' Has left the server', 'None', 'None')
end)