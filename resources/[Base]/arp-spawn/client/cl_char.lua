...
function Login.CreatePlayerCharacterPeds(characterModelData,isReset)
    ...
    events:Trigger("arp-base:fetchPlayerCharacters", nil, function(data)
        if data.err then
            return
        end
...
--[[
    ...
    Description: dealing with arp-base functionality in order to set/get the correct information for chars
]]
...
function Login.getCharacters(isReset)
    ...
    if not isReset then
        ...
        events:Trigger("arp-base:loginPlayer", nil, function(data)
...
function Login.SelectedChar(data)
	...
	local events = exports["arp-base"]:getModule("Events")
	events:Trigger("arp-base:selectCharacter", data.actionData, function(returnData)
        ...
        if Login.CurrentClothing[data.actionData] == nil then
	        ...
	        TriggerEvent("arp-base:firstSpawn")
	    end
    end)
end
...
RegisterNetEvent("arp-spawn:finishedClothing")
AddEventHandler("arp-spawn:finishedClothing", function(endType)
