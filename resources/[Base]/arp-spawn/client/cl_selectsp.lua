...
AddEventHandler('spawn:clientSpawnData', function(spawnData)
		...
		--[[if Spawn.housingCoords == nil then
			Spawn.housingCoords = exports["arp-housing"]:retriveHousingTable();
		end]]
...
RegisterNetEvent("arp-spawn:firstspawnyo")
AddEventHandler("arp-spawn:firstspawnyo", function()
    if GetEntityModel(PlayerPedId()) == GetHashKey("mp_f_freemode_01") then
        TriggerEvent("arp-spawn:finishedClothingstuffbish")
    else
        TriggerEvent("arp-spawn:finishedClothingstuff")
    end
end)

RegisterNetEvent('arp-spawn:finishedClothingstuff')
AddEventHandler('arp-spawn:finishedClothingstuff', function()
...
RegisterNetEvent('arp-spawn:finishedClothingstuffbish')
AddEventHandler('arp-spawn:finishedClothingstuffbish', function()
...
function Spawn.getDevSpawn()
	...
	local devspawn = exports["storage"]:tryGet("vector4","voiddevspawn")
	if devspawn then
...
function Spawn.getBusinessSpawn(pBusinessCode, pBusinessName)
	...
	local hasPermission = RPC.execute("arp-business:hasPermission", pBusinessCode, "key_access")
...
function Spawn.selectedSpawn(spawnInfo)
	print("selectedSpawn", json.encode(spawnInfo))
