...
function Login.NUICallback(data)
	...
	if data.action == "currentHover" then
		...
		local coords = exports["arp-base"]:getModule("Util"):ScreenRelToWorld(GetCamCoord(LoginSafe.Cam), GetCamRot(LoginSafe.Cam,2),offset)
	...
	if data.action == "deleteCharacter" then
		...
		local events = exports["arp-base"]:getModule("Events")
		events:Trigger("arp-base:deleteCharacter", data.actionData, function(deleted)
            Login.getCharacters(true)
        end)
	end
	...
	if data.action == "newCharacter" then
		...
		events:Trigger("arp-base:createCharacter", chardata, function(created)
			...
			if not created then
				...
				TriggerServerEvent('arp-base:charactercreate', chardata.firstname, chardata.lastname, chardata.dob, pGender)
...
RegisterNetEvent("arp-base:spawnInitialized")
AddEventHandler("arp-base:spawnInitialized", function()
	TriggerServerEvent("commands:reset:login")
	TriggerServerEvent("arp-base:clearStates")
	Login.nativeStart()
end)

RegisterNetEvent("arp-base:relog")
AddEventHandler("arp-base:relog", function(type)
	TriggerServerEvent("commands:reset:login")
	TriggerServerEvent("arp-base:clearStates")
	if type == "motel" then
		Login.nativeStart()
