...
RegisterServerEvent("erp-events:listenEvent")
AddEventHandler("erp-events:listenEvent", function(id, name, args)
    local src = source
    ...
    Void.Events.Registered[name].f(Void.Events.Registered[name].mod, args, src, function(data)
        TriggerClientEvent("erp-events:listenEvent", src, id, data)
    end)
end)
