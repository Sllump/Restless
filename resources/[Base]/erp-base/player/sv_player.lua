...
local function AddMethod(player)
    ...
    function player.networkVar(self, var, data)
        ...
        TriggerClientEvent("erp-base:networkVar", GetUser(self):getVar("source"), var, data)
    end
    ...
    function player.addMoney(self, amt)
        ...
        Void.DB:UpdateCharacterMoney(GetUser(self), characterId, cash, function(updatedMoney, err) 
            if updatedMoney then
                ...
                -- exports["erp-base"]:AddLog("Cash Added", GetUser(self), "Money added to user, amount: " .. tostring(amt))
            end
        end)
    end
    ...
    function player.removeMoney(self, amt)
        ...
        Void.DB:UpdateCharacterMoney(GetUser(self), characterId, cash, function(updatedMoney, err) 
            if updatedMoney then
                ...
                TriggerClientEvent("banking:updateCash", GetUser(self).source, GetUser(self):getCash(), amt)
                -- exports["erp-base"]:AddLog("Cash Removed", GetUser(self), "Money removed from user, amount: " .. tostring(amt))
            end
        end)
    end
    ...
    function player.removeBank(self, amt)
        ...
        Void.DB:UpdateCharacterBank(GetUser(self), characterId, bank, function(updatedMoney, err) 
            if updatedMoney then
                ...
                -- exports["erp-base"]:AddLog("Bank Removed", GetUser(self), "Bank removed from user, amount: " .. tostring(amt))
            end
        end)
    end
    ...
    function player.addBank(self, amt)
        ...
        Void.DB:UpdateCharacterBank(GetUser(self), characterId, bank, function(updatedMoney, err) 
            if updatedMoney then
                ...
                TriggerClientEvent("banking:updateBalance", GetUser(self).source, GetUser(self):getBalance(), amt)
                -- exports["erp-base"]:AddLog("Bank Added", GetUser(self), "Bank added to user, amount: " .. tostring(amt))
            end
        end)
    end
...
local pos = {}
RegisterServerEvent('erp-base:updatecoords')
AddEventHandler('erp-base:updateCoords', function(x,y,z)
    local src = source
    pos[src] = {x,y,z}
end)
...
AddEventHandler("playerDropped", function(reason)
    ...
    local pUser = exports["erp-base"]:getModule("Player"):GetUser(src)
    ...
    if userjob == "police"or userjob == "sheriff" or userjob == "state" then
        exports['erp-emergencyjobs']:DisconnectJobUpdater(userjob)
        TriggerEvent("erp-emergencyjobs:log:update", src, "clockoff")
    elseif userjob == "ems" then
        exports['erp-emergencyjobs']:DisconnectJobUpdater(userjob)
    end

    Void.Users[src] = nil
    TriggerEvent('erp-base:playerDropped', src, user)
    
    exports['erp-admin']:addPlayerLog('Player Left', GetPlayerIdentifier(src, 0), pSteamName..' Has left the server', 'None', 'None')
end)