...
function updateCarTable(model,price,name)
	for i=1, #carTable do
		...
		TriggerServerEvent("erp-pdm:CarTablePDM",carTable)
	end
end

local myspawnedvehs = {}

RegisterNetEvent("erp-pdm:testdrive")
AddEventHandler("erp-pdm:testdrive", function()
	if exports["erp-business"]:IsEmployedAt("pdm") or #(vector3(-23.69871, -1094.529, 27.305231) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("erp-pdm:EnableBuy")
AddEventHandler("erp-pdm:EnableBuy", function()
	if #(vector3(-23.69871, -1094.529, 27.305231) - GetEntityCoords(PlayerPedId())) > 50.0 and exports["erp-business"]:IsEmployedAt("pdm") then
		return
	end
	...
	TriggerServerEvent("erp-pdm:BuyEnabledSV",vehplate)
end)

RegisterNetEvent("erp-pdm:BuyEnabledCL")
AddEventHandler("erp-pdm:BuyEnabledCL", function(addplate)
	buyPlate[addplate] = true
	Citizen.Wait(60000)
	buyPlate[addplate] = nil
end)

RegisterNetEvent("erp-pdm:PDMCommission")
AddEventHandler("erp-pdm:PDMCommission", function(newAmount)
	if eexports["erp-business"]:IsEmployedAt("pdm") or #(vector3(-23.69871, -1094.529, 27.305231) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("erp-pdm:ReturnPDMTTable")
AddEventHandler("erp-pdm:ReturnPDMTTable", function(newTable)
	carTable = newTable
	DespawnSaleVehicles()
	SpawnSaleVehicles()
end)
...
function AttemptBuy(tableid)
	...
	TaskWarpPedIntoVehicle(PlayerPedId(),veh,-1)
	TriggerEvent("vehicle:keys:addNew",veh, plate)
	TriggerServerEvent('erp-pdm:ChechMoney',name, model, price)
	commissionbuy = (baseprice * commission / 100)
	Citizen.Wait(100)
	exports['erp-interface']:hideInteraction()
	TriggerServerEvent('erp-banking:business_money', price, 'pdm', 'add')
end
		...
		if #(vector3(carspawns[i]["x"],carspawns[i]["y"],carspawns[i]["z"]) - GetEntityCoords(PlayerPedId())) < 1.5 then
			...
			if exports["erp-business"]:IsEmployedAt("pdm") then
				if buyPlate[addplate] ~= nil then
					exports['erp-interface']:showInteraction("$" .. math.ceil(price) .. " | Com: %" ..commission.. " | [E] to change | [G] to buy.")
				else
					exports['erp-interface']:showInteraction("$" .. math.ceil(price) .. " | Com: %" ..commission.. " | [E] to change")
				end
			else
				if buyPlate[addplate] ~= nil then
					exports['erp-interface']:showInteraction("$" .. math.ceil(price) .. " [G] to buy.")
				else
					exports['erp-interface']:showInteraction("Buy Price: $" .. math.ceil(price) .. " ")
				end
			end
...
function SpawnSaleVehicles()
	if not hasspawned then
		TriggerServerEvent("erp-pdm:RequestPDMTTable")
		Citizen.Wait(1500)
	end
...
function CloseCreator(name, veh, price, plate)
	Citizen.CreateThread(function()
		...
		if not boughtcar then
			...
			-- local VehicleProps = exports['erp-base']:FetchVehProps(personalvehicle)
			...
			--TriggerServerEvent('erp-pdm:BuyVehicle', plate, name, vehicle, price) -- make this rpc, and await response
			local result = RPC.execute("erp-pdm:BuyVehicle", vehicle, tbl)
			DespawnSaleVehicles()
			SpawnSaleVehicles()
			Citizen.Wait(100)
			exports['erp-interface']:hideInteraction()
...
RegisterNetEvent("erp-pdm:FailedPurchase")
AddEventHandler("erp-pdm:FailedPurchase", function()
...
function ButtonSelected(button)
	...
	elseif this == "jobvehicles" or this == "s-class" or this == "a-class" or this == "b-class" or this == "c-class" or this == "casino" or this == "d-class" or this == "offroad" or this == "suvs" or this == "vans" or this == "industrial" or this == "cycles" or this == "motorcycles" then
		if ownerMenu then
			...
			TriggerServerEvent('erp-pdm:ChechMoney',button.name, button.model, button.costs)
		end
	end
end
	...
	while true do
		...
		if vehshop.opened then
			...
			for i,button in pairs(menu.buttons) do
				...
				if exports["erp-business"]:IsEmployedAt("pdm") and i >= vehshop.menu.from and i <= vehshop.menu.to then
