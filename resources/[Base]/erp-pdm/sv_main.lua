...
RegisterServerEvent('erp-pdm:CarTablePDM')
AddEventHandler('erp-pdm:CarTablePDM', function(table)
    if table ~= nil then
        carTable = table
        TriggerClientEvent('erp-pdm:ReturnPDMTTable', -1, carTable)
        for i=1, #carTable do
...
RegisterServerEvent('erp-pdm:FinaceEnabledSV')
AddEventHandler('erp-pdm:FinaceEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('erp-pdm:FinaceEnabledCL', -1, plate)
    end
end)

RegisterServerEvent('erp-pdm:BuyEnabledSV')
AddEventHandler('erp-pdm:BuyEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('erp-pdm:BuyEnabledCL', -1, plate)
    end
end)

-- return table
-- TODO (return db table)
RegisterServerEvent('erp-pdm:RequestPDMTTable')
AddEventHandler('erp-pdm:RequestPDMTTable', function()
    local user = source
    exports.oxmysql:execute("SELECT * FROM vehicle_display", {}, function(display)
        ...
        TriggerClientEvent('erp-pdm:ReturnPDMTTable', user, carTable)
    end)
end)

-- Check if player has enough money
RegisterServerEvent('erp-pdm:ChechMoney')
AddEventHandler('erp-pdm:ChechMoney', function(name, model, price)
    local src = source
    ...
    elseif tonumber(cash) <= price then
        ...
        TriggerClientEvent('erp-pdm:FailedPurchase', src)
    end
end)

RPC.register('erp-pdm:BuyVehicle', function(pSource, pModel, pCoords)
    local src = pSource
    local user = exports["erp-base"]:getModule("Player"):GetUser(src)
    ...
    local vehicleInfo = exports["erp-vehicles"]:GenerateVehicleInfo(src, char.id, pModel.param, 'owned', 'pdm')
    ...
    local spawned = exports["erp-vehicles"]:SpawnPlayerVehicle(src, vehicleInfo.vin, newCoords, heading, false)
