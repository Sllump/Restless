...
function Login.CreatePlayerCharacterPeds(characterModelData,isReset)
    ...
    events:Trigger("erp-base:fetchPlayerCharacters", nil, function(data)
        if data.err then
            return
        end
...
--[[
    ...
    Description: dealing with erp-base functionality in order to set/get the correct information for chars
]]
...
function Login.getCharacters(isReset)
    ...
    if not isReset then
        ...
        events:Trigger("erp-base:loginPlayer", nil, function(data)
...
RegisterNetEvent("erp-spawn:finishedClothing")
AddEventHandler("erp-spawn:finishedClothing", function(endType)
