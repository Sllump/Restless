...
function Login.NUICallback(data)
	...
	if data.action == "currentHover" then
		...
		local coords = exports["erp-base"]:getModule("Util"):ScreenRelToWorld(GetCamCoord(LoginSafe.Cam), GetCamRot(LoginSafe.Cam,2),offset)
	...
	if data.action == "deleteCharacter" then
		...
		events:Trigger("erp-base:deleteCharacter", data.actionData, function(deleted)
            Login.getCharacters(true)
        end)
	end
...
RegisterNetEvent("erp-base:spawnInitialized")
AddEventHandler("erp-base:spawnInitialized", function()
	TriggerServerEvent("commands:reset:login")
	TriggerServerEvent("erp-base:clearStates")
	Login.nativeStart()
end)

RegisterNetEvent("erp-base:relog")
AddEventHandler("erp-base:relog", function(type)
	TriggerServerEvent("commands:reset:login")
	TriggerServerEvent("erp-base:clearStates")
	if type == "motel" then
		Login.nativeStart()
