Ethereal.BlipManager = Ethereal.BlipManager or {}
Ethereal.Blips = Ethereal.Blips or {}

function Ethereal.BlipManager.CreateBlip(id, data)
    ...
    Ethereal.Blips[id] = {blip = blip, data = data}
end

function Ethereal.BlipManager.RemoveBlip(id)
    local blip = Ethereal.Blips[id]
    if blip then RemoveBlip(blip.blip) end
    Ethereal.Blips[id] = nil
end

function Ethereal.BlipManager.HideBlip(id, toggle)
    local blip = Ethereal.Blips[id]
    if not blip then return end
    if toggle then SetBlipAlpha(blip, 0) else SetBlipAlpha(blip, 255) end
end

function Ethereal.BlipManager.GetBlip(id)
    local blip = Ethereal.Blips[id]
