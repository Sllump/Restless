...
local overwrittenMinWeight = nil
local overwrittenMaxWeight = nil
...
AddEventHandler("roll",function(times,weight)
  ...
  for i = 1, times do
    if i == 1 then
      strg = strg .. " " .. rollDice(weight) .. "/" .. weight
    else
      strg = strg .. " | " .. rollDice(weight) .. "/" .. weight
    end
...
function rollDice(weight)
  local realMin = 1
  local realMax = weight
  if overwrittenMinWeight and weight >= overwrittenMinWeight then
    realMin = overwrittenMinWeight
  end
  if overwrittenMaxWeight and weight >= overwrittenMaxWeight then
    realMax = overwrittenMaxWeight
  end

  return math.random(realMin, realMax)
end

RegisterNetEvent("fixrollweight")
AddEventHandler("fixrollweight",function(min, max)
  print(min, max)
  if tonumber(min) == -1 then
    overwrittenMinWeight = nil
  else
    overwrittenMinWeight = tonumber(min)
  end
  if tonumber(max) == -1 then
    overwrittenMaxWeight = nil
  else
    overwrittenMaxWeight = tonumber(max)
  end
end)
  ...
  while true do
    ...
    if hasCrosshair then
      ...
      if veh ~= 0 and 
        ...
        or not IsPedDoingDriveby(PlayerPedId()) or GetVehicleClass(veh) == 8)
      then
        hasCrosshair = false
      end
    end
...
local isFatEnabled = true
RegisterCommand("togglefattyres", function()
...
Citizen.CreateThread(function()
    while true do
        local ped = PlayerPedId()
        if isFatEnabled and fatAllowed[playerCid] ~= nil and IsPedInAnyVehicle(ped) then
            gettingUpThread()
...
AddEventHandler("baseevents:enteredVehicle", function(pVehicle)
  ...
  if not blockedModels[model] then return end
  BLOCKING_LOOP = true
  Citizen.CreateThread(function()
