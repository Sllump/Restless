...
const registerProcedure = (name: string, callback: Function) => {
    ...
    Events.onNet(`__npx_req:${name}`, async (payload: any, args: any) => {
        ...
        if (decoded.type === 'remote') {
            Events.emitNet(`__npx_res:${decoded.origin}`, decoded.id, [result, cb]);
        } else if (decoded.type === 'local') {
            Events.emit(`__npx_res:${decoded.origin}`, decoded.id, [result, cb]);
        }
    });
}
...
const executeProcedure = (name: string, ...args: any[]) => {
    ...
    Events.emitNet(`__npx_req:${name}`, EventUtils.encode(payload), promise.finally(() => executed.delete(payload.id)));

    return promise;
}

Events.onNet(`__npx_res:${ResourceName}`, (id: any, result: any) => {
