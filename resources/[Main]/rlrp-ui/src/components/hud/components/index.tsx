...
const Hud: React.FC = () => {
    ...
    const [wasCoughHidden, setWasCoughHidden] = useState(false)
    ...
    /* Cough */
    const [coughDisplay, setCoughDisplay] = useState(false)
    const [coughOpacity, setCoughOpacity] = useState(1)
    const [coughArray, setCoughArray] = useState("125.66370614359172")
    const [coughOffset, setCoughOffset] = useState("125.66370614359172")
        ...
        if (app === "hud") {
            ...
            if (appData.displayAllForce !== undefined) {
                if (appData.displayAllForce === true) {
                    ...
                    if (!coughDisplay) {
                        setCoughDisplay(true)
                        setCoughOpacity(1)
                        setWasCoughHidden(true)
                    }
                    if (!oxygenDisplay) {
            ...
            <Grid container className={classes.root}>
                <div className={classes.hudOuterContainer} style={{ display: showHud ? 'flex' : 'none' }}>
                    ...
                    <div className={classes.hudInnerContainer} style={{ left: defaultMinimap ? '0.4vw' : '0px' }}>
                        ...
                        {/* Cough */}
                        <div className={classes.hudIconWrapper} style={{ opacity: coughOpacity, transition: 'all 1s linear 1s', maxWidth: coughDisplay ? '54px' : '0px', display: coughDisplay ? 'flex' : 'none' }}>
                            <div className={classes.hudIcon}>
                                <svg version="1.1" xmlns="http://www.w3.org/2000/svg" style={{ height: '48px', width: '48px', transform: 'rotate(-90deg)', zIndex: 100 }}>
                                    <circle r="20" cx="24" cy="24" fill="transparent" stroke={'#4F9EC5'} stroke-width="6" stroke-dasharray="125.66370614359172" stroke-dashoffset="0" stroke-opacity={'1'} style={{ transition: 'stroke-dashoffset 400ms linear 0s', willChange: 'transition' }} />
                                </svg>
                            </div>
                            <div className={classes.hudIcon}>
                                <svg version="1.1" xmlns="http://www.w3.org/2000/svg" style={{ height: '48px', width: '48px', transform: 'rotate(-90deg)', zIndex: 200 }}>
                                    <circle r="20" cx="24" cy="24" fill="transparent" stroke="#4F9EC5" stroke-width="6" stroke-dasharray={coughArray} stroke-dashoffset={coughOffset} stroke-opacity="1" style={{ transition: 'stroke-dashoffset 400ms linear 0s', willChange: 'transition' }} />
                                </svg>
                            </div>
                            <div className={classes.hudIcon}>
                                <div className={classes.iconWrapper}>
                                    <i className="fas fa-head-side-cough fa-w-11 fa-fw" style={{ color: 'white', fontSize: '18px' }}></i>
                                </div>
                            </div>
                        </div>
