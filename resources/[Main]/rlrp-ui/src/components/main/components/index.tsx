...
export function RestartAlert() {
  ...
  const Text: any = 'Full restart in progress...'
  const Type: any = 'info'
    ...
    <>
      <div style={{position: 'absolute', display: ShowAlert ? '' : 'none'}} className={classes.restartContainer}>
        <div className={classes.restartsContainer}>
          <Alert sx={{boxShadow:'1px 1px 1px rgba(0, 0, 0, 0.644)', color:'white'}} variant="filled" severity={Type} className={'info' === Type ? "info alert" : 'alert'}>
            {Text}
          </Alert>
        </div>
      </div>
