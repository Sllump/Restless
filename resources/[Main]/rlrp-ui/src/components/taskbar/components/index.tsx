...
const TaskBar: React.FC = () => {
    ...
    const [Circle, SetCircle]: any = useRecoilState(hudCircleTaskbarEnabled)
    const [TaskId, SetTaskId]: any = useState('')
    const [Label, SetLabel]: any = useState('Test')
    const [duration, SetDuration]: any = useState(0)
    const [Value, SetValue]: any = useState(50)
    ...
    useEffect(() => {
      if (
        ...
        var dvex1 = 40 * Math.PI,
          dvex2 = dvex1 - ((100 * Value) / 100 / 50) * dvex1
    ...
    return (
      <>
        <div style={{display: ShowTaskBar ? '' : 'none'}} className={classes.taskbarOuterContainer}>
          <div className={classes.taskbarFlex}></div>
          <div className={classes.taskbarInnerContainer}>
            <div style={{ display: Circle ? '' : 'none' }} className={classes.taskbarFlex}></div>
                ...
                <div className={classes.taskbarCircleContainer}>
                  <div className={classes.taskbarSvg}>
                    <svg version='1.1' xmlns='http://www.w3.org/2000/svg' style={{ height: '48px', width: '48px', transform: 'rotate(-90deg)',filter: 'drop-shadow(rgba(0, 0, 0, 0.4) 0px 0px 2px)' }}>
                      <circle r='20' cx='24' cy='24' fill='transparent' stroke='rgba(255, 255, 255, 0.5)' stroke-width='6' stroke-dasharray='125.66370614359172' stroke-dashoffset='0' style={{transition: 'stroke-dashoffset 600ms linear 0s', willChange: 'transition'}}></circle>
                    </svg>
                  </div>
                  <div className={classes.taskbarSvg}>
                    <svg version='1.1' xmlns='http://www.w3.org/2000/svg' style={{ height: '48px', width: '48px', transform: 'rotate(-90deg)',filter: 'drop-shadow(rgba(0, 0, 0, 0.4) 0px 0px 2px)' }}>
                      <circle r='20' cx='24' cy='24' fill='transparent' stroke='#4F7CAC' stroke-width='6' stroke-dasharray={CircumFerence}stroke-dashoffset={Offset} style={{transition: 'stroke-dashoffset 600ms linear 0s', willChange: 'transition'}}></circle>
                    </svg>
                  </div>
                </div>
                <div>
            ...
            <div style={{ display: Circle ? 'none' : 'flex' }} className={classes.taskbarInnerContainer}>
              <div className={classes.taskbarNormalContainer}>
                ...
                <div className={classes.taskbarSliderContainer} style={{width: ''+Value+'%', transition: 'width 0ms linear 0s'}}></div>
              </div>
            </div>
          </div>
        </div>
