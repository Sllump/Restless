Boosting = {
    StartBoost = function(data)
        local Contract = RPC.execute('erp_laptop:boosting:fetchContractData', data)

        if data.Type == 'Regular' then
            local Crypto = RPC.execute('erp_laptop:boosting:payCrypto', Contract.cost / 2)
        end

        if data.Type == 'Vin Scratch' then
            local Crypto = RPC.execute('erp_laptop:boosting:payCrypto', Contract.cost)
        end
