...
RPC.register('erp_laptop:gangs:addMember', function(source, data)
    local src = source
    if Gangs.CheckInGang(src, data.stateId) then return 'In Gang' end
    Actions.AddMember(src, data)
end)

RPC.register('erp_laptop:gangs:kickMember', function(source, data)
    Actions.KickMember(source, data)
end)

RPC.register('erp_laptop:gangs:setRank', function(source, data)
