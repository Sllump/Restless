Cornering = {
    ...
    startCornering = function(Entity, Drug)
        ...
        TriggerEvent('erp_ui:sentNotification', 'info', 'Corner Active', 5000)
        ...
        Citizen.CreateThread(function()
            ...
            while Cornering.CurrentlyCornering do
                ...
                if #(Cornering.CurrentCornerCoords - plyCoords) > 50.0 then
                    TriggerEvent('erp_ui:sentNotification', 'error', 'No longer selling...', 5000)
                ...
                if not foundPed then
                    ...
                    RPC.execute("erp_laptop:cornering:createPed", coords, NetworkGetNetworkIdFromEntity(foundPed))
                end

                if notFoundCount > 7 then
                    TriggerEvent('erp_ui:sentNotification', 'error', 'Looks like this spot has dried up.', 5000)
                    Cornering.stopCornering()
                    return
                end
    ...
    doSale = function(pEntity)
        exports['erp_target']:RemoveTargetEntity(pEntity, "Sell")
        local DrugAmount = math.random(1, 5)

        local hasBaggies = exports['erp_inventory']:hasEnoughOfItem(Cornering.CorneringDrug, DrugAmount, true)
        if not hasBaggies then
            TriggerEvent('erp_ui:sentNotification', 'error', 'You need more baggies to sell.', 5000)
            return
        end
    
        if exports['erp_inventory']:hasEnoughOfItem(Cornering.CorneringDrug, DrugAmount) then
            TriggerEvent('inventory:removeItem', Cornering.CorneringDrug, DrugAmount)
            RPC.execute('erp_crime:weed:syncCornerHandOff', Cornering.CurrentCornerCoords, NetworkGetNetworkIdFromEntity(pEntity), DrugAmount)
            cleanMoney()
        end
        ...
        -- if RNG == 2 then
        --     RPC.execute("erp_dispatch:addCall", "10-31A", "Investigate Suspicious Actitity", {}, {GetEntityCoords(PlayerPedId())[1], GetEntityCoords(PlayerPedId())[2], GetEntityCoords(PlayerPedId())[3]}, 102, 480, 0)
        -- end
    end
}

RegisterNetEvent("erp_laptop:cornering:createdCornerPed")
AddEventHandler("erp_laptop:cornering:createdCornerPed", function(Ped, Coords)
    if NetworkHasControlOfEntity(NetworkGetEntityFromNetworkId(Ped)) then

        exports['erp_target']:AddTargetEntity(NetworkGetEntityFromNetworkId(Ped), {
            options = {
                {
                    label = "Sell",
