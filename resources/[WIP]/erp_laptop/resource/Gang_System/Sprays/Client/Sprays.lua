...
RegisterCommand('spray', function()
    TriggerEvent('erp_laptop:gangs:startSpraying')
end)

RegisterNetEvent("erp_laptop:gangs:startSpraying")
AddEventHandler("erp_laptop:gangs:startSpraying", function(item, object)
    local SprayObject = nil

    if not Sprays.currentlyplacing then
        local InGang = RPC.execute('erp_laptop:gangs:checkInGang')
        if not InGang then TriggerEvent('erp_ui:sentNotification', 'error', 'You have no use for this...', 5000) return end

        -- Check if gang flag is nearby or a spray is..
        if not GangSystem.Functions.SprayCheck() then

            local SprayModel = RPC.execute('erp_laptop:gangs:getSprayModel')

            SprayObject = SprayModel

            TriggerEvent('erp_ui:sentNotification', 'info', '[E] Place | ESC Cancel', 5000)
            ...
            while Sprays.currentlyplacing do
                ...
                if IsControlJustPressed(0, 38) then
                    if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), x, y, z, false) > 2.0 then
                        TriggerEvent('erp_ui:sentNotification', 'error', 'Move closer to the place you are trying to spray and try again.', 5000)
                        DeleteEntity(propSpawned)
                        Sprays.currentlyplacing = false
                        return
                    end
                    ...
                    --     DeleteEntity(propSpawned)
                    --     RPC.Execute('erp_laptop:gangs:addGangflag', item.data.gangname, item.data.gangid, { x = coords.x, y = coords.y, z = coords.z, h = heading }, item.data.gangcolor, item.data.ganglogo, GetHashKey(SprayObject))
                    ...
                    -- elseif item.name == "tool_spray_can" then
                        ...
                        local finished = exports['erp_taskbar']:progressCircle({
                            duration = 40000,
                            position = 'bottom',
                            label = 'Spraying Wall...',
                            useWhileDead = false,
                        ...
                        if finished then
                            ...
                            RPC.execute("erp_laptop:gangs:syncPlaceSpray", {
                                Spray = SprayObject,
                                Coords = {
            ...
            TriggerEvent('erp_ui:sentNotification', 'error', 'You cant place a spray here.', 5000)
            Sprays.currentlyplacing = false
            SetEntityDrawOutline(propSpawned, false)
            DeleteEntity(propSpawned)
...
RegisterNetEvent('erp_laptop:gangs:syncClientSpray')
AddEventHandler('erp_laptop:gangs:syncClientSpray', function(Array)
...
AddEventHandler('erp_spawnselect:SetEverything', function()
    ...
    RPC.execute('erp_laptop:gangs:getAllSprays')
    ...
    NUI.sendReactMessage('erp_laptop:closeAllApps')
end)

RegisterNetEvent('erp_laptop:gangs:updateAllSprays')
AddEventHandler('erp_laptop:gangs:updateAllSprays', function()
...
RegisterNetEvent('erp_charselect:initiateCharSelect')
AddEventHandler('erp_charselect:initiateCharSelect', function()
    for k, v in ipairs(Sprays.Active) do
        if DoesEntityExist(v.EntityId) then
            DeleteEntity(v.EntityId)
        end
