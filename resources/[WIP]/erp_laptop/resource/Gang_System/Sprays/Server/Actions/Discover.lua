RPC.register('erp_laptop:gangs:discoverSpray', function(source, Array)
    local src = source
...
RPC.register('erp_laptop:gangs:alreadyDiscovered', function(source, Id)
    local src = source
    local Gang = Gangs.FetchGang(src)
...
RPC.register('erp_laptop:gangs:toggleDiscoveredGraffitis', function(source)
    local src = source
    local Gang = Gangs.FetchGang(src)
    ...
    for k, v in ipairs(Sprays) do
        ...
        TriggerClientEvent('erp_laptop:gangs:addMapSpray', src, v, SprayModel[1].Model)
    end
end)

RPC.register('erp_laptop:gangs:getSprayModelFromId', function(source, Id)
