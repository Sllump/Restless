Citizen.CreateThread(function()
    while GetResourceState('erp_inventory') ~= 'started' do
        Citizen.Wait(500)
    end

    exports['erp_inventory']:registerItem('laptop', function(item, token)
        if Item ~= "laptop" then return end

        NUI.Display = not NUI.Display
        Animation()
        NUI.sendReactMessage('erp_laptop:toggleLaptop')
...
RegisterNUICallback('erp_laptop:closeLaptop', function(data, cb)
